...
int main()
{
    ...
    // load textures
    // -------------
    char cubePath[] = "resources/textures/marble.jpg";
    char floorPath[] = "resources/textures/metal.png";
    // char grassPath[] = "resources/textures/grass.png";
    char grassPath[] = "resources/textures/blending_transparent_window.png"; // 直接借用草的变量名
    unsigned int cubeTexture = loadTexture(cubePath);
    unsigned int floorTexture = loadTexture(floorPath);
    unsigned int grassTexture = loadTransparentTextrue(grassPath);

    // 草的位置
    // 这个顺序会影响深度测试，从而使有些玻璃是透明的，有些不是
